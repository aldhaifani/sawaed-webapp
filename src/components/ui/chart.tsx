...
function ChartContainer({
  ...
  const baseId = (id ?? uniqueId).toString();
  const chartId = `chart-${baseId.replace(/:/g, "")}`;
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const payloadFill =
            ...
            payloadObj !== null &&
            "fill" in payloadObj
              ? (payloadObj as Record<string, unknown>).fill
              : undefined;
                ...
                (() => {
                  ...
                  const safeValue: string | number =
                    ...
                    typeof item.value === "string"
                      ? item.value
                      : "";
                  const safeItem: Payload<ValueType, NameType> = item;
                  const rawPayload =
                    item && typeof item === "object" && "payload" in item
                      ? (item as { payload?: unknown }).payload
                      : undefined;
                  const safePayload: Payload<ValueType, NameType>[] =
                    Array.isArray(rawPayload)
                      ? (rawPayload as Payload<ValueType, NameType>[])
                      : [];
                  return formatter(
                    safeValue,
                    safeName,
  ...
  return (
    ...
    >
      {payload.map((item) => {
        ...
        const entry = item as unknown as LegendEntry;
        const key: string =
          nameKey ??
