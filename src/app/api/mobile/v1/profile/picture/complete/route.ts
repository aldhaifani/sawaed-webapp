...
const BodySchema = z.object({ storageId: z.string().min(1) });

const callMutation = fetchMutation as unknown as (
  name: "profiles:setProfilePictureFromStorageId",
  args: unknown,
...
export async function POST(req: Request): Promise<Response> {
  return Sentry.startSpan(
    { op: "http.route", name: "POST /api/mobile/v1/profile/picture/complete" },
    async () => {
      return withConvexToken(req, async ({ token }) => {
        ...
        try {
          ...
          const res = await callMutation(
            "profiles:setProfilePictureFromStorageId",
