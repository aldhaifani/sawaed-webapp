...
const VerifyOtpSchema = z.object({
  ...
  code: z.string().regex(/^\d{6}$/u, "code_must_be_6_digits"),
});
...
export async function POST(req: Request): Promise<Response> {
  return Sentry.startSpan(
    { op: "http.route", name: "POST /api/mobile/auth/verify-otp" },
    async () => {
      try {
        if (isCorsRequest(req)) {
          return new Response("Invalid origin", { status: 403 });
        }
        const body = await parseJsonBody(req, VerifyOtpSchema);
        const args = {
          provider: "resend-otp",
          params: { email: body.email, code: body.code },
        };
        const result = await callAction("auth:signIn", args);
        const tokens = extractTokens(result);
        if (!tokens) {
          return respondError("invalid_code", "Invalid or expired code", 400);
        }
        return NextResponse.json(tokens);
      } catch (err) {
        Sentry.captureException(err);
        const message =
          err instanceof Error ? err.message : "verify_otp_failed";
        return respondError("verify_otp_failed", message, 400);
      }
    },
  );
}
